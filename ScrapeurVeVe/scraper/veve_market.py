...
MARKET_PSEUDOS_HEADER = ["seller_id", "seller_name", "wallet_imx", "wallet_stackr",
                         "matched_by", "first_seen", "last_seen", "listings_seen"]
STACKR_PSEUDOS_TAB = "Pseudos"
...
def _load_stackr_wallets(sh):
    """Indexes {veve_user_id -> row} and {username_lower -> row} from the StackR
    Pseudos tab, to attach a wallet to each market seller."""
    by_user: Dict[str, Dict[str, Any]] = {}
    by_name: Dict[str, Dict[str, Any]] = {}
    try:
        ws = sh.worksheet(STACKR_PSEUDOS_TAB)
    except Exception:
        return by_user, by_name
...
def _merge_pseudos(sheet_id: str, seen: Dict[str, Dict[str, Any]]) -> tuple:
    """Accumulate sellerId <-> sellerName and attach a wallet (via the StackR
    Pseudos tab, matched by veve_user_id then username). Returns (new, matched)."""
    sh = _client().open_by_key(sheet_id)
    ws = _open_worksheet(sh, MARKET_PSEUDOS_TAB, cols=len(MARKET_PSEUDOS_HEADER))
    by_user, by_name = _load_stackr_wallets(sh)
    ...
    for sid, info in seen.items():
        ...
        if row:
            row["seller_name"] = info["seller_name"]
            row["last_seen"] = now
            row["listings_seen"] = int(row.get("listings_seen", 0) or 0) + info["count"]
        else:
            row = existing[sid] = {"seller_id": sid, "seller_name": info["seller_name"],
                                   "first_seen": now, "last_seen": now,
                                   "listings_seen": info["count"]}
            new += 1
        # attach / refresh wallet from StackR (user_id first, then username)
        w = by_user.get(sid) or by_name.get(str(info["seller_name"]).strip().lower())
        if w:
            row["wallet_imx"] = w.get("wallet_imx", "")
            row["wallet_stackr"] = w.get("wallet_stackr", "")
            row["matched_by"] = "user_id" if sid in by_user else "username"

    matched = sum(1 for r in existing.values() if str(r.get("wallet_imx", "")).strip())
...
def main() -> int:
    ...
    new_pseudos, matched_wallets = _merge_pseudos(sheet_id, pseudos)
